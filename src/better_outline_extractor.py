...
def clean_text(text):
    ...
    text = re.sub(r'(\w)\s+(\w)', lambda m: m.group(1) + m.group(2)
                  if len(m.group(1)) > 2 and len(m.group(2)) > 2 else m.group(0), text)
    return text.strip()
...
def extract_title(page):
    ...
    for block in blocks:
        block_pos = block.get("bbox", (0, 0))[1]
        for line in block.get("lines", []):
            line_texts = []
            ...
            if line_has_title:
                ...
                if (previous_block_pos is not None and abs(block_pos - previous_block_pos) < 20 and len(current_title_block) > 0):
...
def group_multiline_headings(heading_candidates):
    ...
    for candidate in heading_candidates[1:]:
        ...
        if (size == current_group[0] and page == current_group[2] and
                (text[0].islower() or len(current_group[1].split()[-1]) <= 3 or len(text.split()[0]) <= 3)):
            if (current_group[1].endswith('-') or len(current_group[1].split()[-1]) <= 2 or len(text.split()[0]) <= 2):
